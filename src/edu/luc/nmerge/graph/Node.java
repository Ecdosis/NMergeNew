...
public class Node
{
	...
	/**
	 * Split a Node into two, adding an empty arc between the
	 * two nodes. Copy all the outgoing arcs to the right node and 
	 * all the incoming arcs to the left node. Add an arc carrying 
	 * all the node's versions in between
	 * @return the right hand node
	 */
	public Node split() throws Exception
	{
		...
		outgoing.toArray( array );
		for ( int i=0;i<array.length;i++ )
		{
			removeOutgoing( array[i] );
			right.addOutgoing( array[i] );
		}
		addOutgoing( a );
		right.addIncoming( a );
		return right;
	...
	{
		ArrayList<Arc> constrainedArcs = new ArrayList<Arc>();
   ...
   /**
	...
	{
		ArrayList<PrevChar> array = new ArrayList<PrevChar>();
