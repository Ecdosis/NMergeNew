...
public class SuffixTree 
{
	...
	*/
	 ...
	 * @throws MVDException
	 */
	int SEA( Pos pos, Path str, char afterRule3, int ruleApplied ) throws MVDException
	{
		...
		if ( debug )
		{
			...
			if ( afterRule3 == 0 )
				System.out.print("   followed from ("+pos.node.edgeLabelStart+","+
					getNodeLabelEnd(pos.node)+" | "+pos.edgePos+") ");
			else
				System.out.print("   starting at ("+pos.node.edgeLabelStart+","
					+getNodeLabelEnd(pos.node)+" | "+pos.edgePos+") ");
		}
	 ...
	 * @throws MVDException
	 */
	void SPA( Pos pos, int phase, ExtensionParam ep )  throws MVDException       
	{
	 ...
	 * is increased.
	 */
	void printNode( Node node1, int depth )
	{
	   ...
	   if ( depth > 0 )
	   {
	      ...
	      while ( start <= end )
	      {
	         System.out.print( (char)treeString[start] );
	         start++;
	      }
	      if ( debug )
	         System.out.print("  \t\t\t("+node1.edgeLabelStart+","
	        	+end+" | "+node1.pathPosition+")");
	      System.out.print("\n");
	   }
	...
	boolean selfTest()
	{
	   ...
	   // if we are here no search has failed and the test passed successfully
